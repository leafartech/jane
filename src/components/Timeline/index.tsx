...
export default function Timeline({ texts }: TimelineProps) {
    return (
        <ul className="w-full max-w-2xl relative flex flex-col gap-6 pb-0 ps-10 sm:ps-12">
            ...
            </svg>
            {
                texts.map((text, index) => (
                    <li className="relative mb-12" key={index}>
                        ...
                        <p className="text-zinc-200 inter">{text.text}</p>
                    </li>
                ))
            }
        </ul>
